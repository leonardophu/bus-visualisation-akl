...
function loadBusIcons() {
    // Load the custom icon image
    console.log("before loading images")
    map.loadImage('/BusImages/OnTimeBus.png', (error, image) => {
        console.log("loading images");
        if (error) throw error;
        map.addImage('OnTimeBus', image); // Add the custom icon as an image resource
        console.log("loaded images");
    });
    // Load the custom icon image
    map.loadImage('/BusImages/GhostBus.png', (error, image) => {
        if (error) throw error;
        map.addImage('GhostBus', image); // Add the custom icon as an image resource
    });

    // Load the custom icon image
    map.loadImage('/BusImages/VeryLate.png', (error, image) => {
        if (error) throw error;
        map.addImage('VeryLateBus', image); // Add the custom icon as an image resource
        console.log("Hello VeryLate");
    });

    // Load the custom icon image
    map.loadImage('/BusImages/SlightlyLate.png', (error, image) => {
        if (error) throw error;
        map.addImage('SlightlyLateBus', image); // Add the custom icon as an image resource
    });
