...
even_split = function(x, y) {
  base_value = x %/% y       # Integer division
  remainder = x %% y         # Remainder
  
  # Create a vector filled with base values
  bins = rep(base_value, y)
...
interpolation = function(trip_data, date_) {
  start_time = as.numeric(as.POSIXct(paste0(date_, "04:00:00"), format="%Y-%m-%d %H:%M:%S", tz="UTC"))
  end_time = as.numeric(as.POSIXct(paste0(date_, "23:59:59"), format="%Y-%m-%d %H:%M:%S", tz="UTC"))
  
  tryCatch({
    storage_path = "../../interpolation/"
    ...
    for(i in 1:(nrow(trip_data) - 1)) {
      ...
      start = which(shapes_file$shape_pt_lat == trip_data$stop_lat[i] & shapes_file$shape_pt_lon == trip_data$stop_lon[i])
      end = which(shapes_file$shape_pt_lat == trip_data$stop_lat[i+1] & shapes_file$shape_pt_lon == trip_data$stop_lon[i+1])
      ...
      interpolated_point = st_line_sample(lines, sample = seq(0,1, length.out = trip_data$timestamps[i+1] - trip_data$timestamps[i] + 1))
